...
// Класс для хранения графа и выполнения алгоритма Форда-Фалкерсона
class Graph {
public:
Graph(int n) {
this->n = n;
...
// Выполнение алгоритма Форда-Фалкерсона
int maxFlow(int s, int t) {
int flow = 0;
while (true) {
QVector<int> parent(n, -1);
QVector<int> minCapacity(n, INF);
std::queue<int> q;
q.push(s);
parent[s] = -2;
minCapacity[s] = INF;
...
int e = adj[u][i];
int v = edges[e].to;
if (parent[v] == -1 && edges[e].capacity > edges[e].flow) {
parent[v] = e;
minCapacity[v] = std::min(minCapacity[u], edges[e].capacity - edges[e].flow);
if (v == t) {
flow += minCapacity[t];
int curr = t;
while (curr != s) {
int e = parent[curr];
edges[e].flow += minCapacity[t];
edges[e ^ 1].flow -= minCapacity[t];
curr = edges[e].from;
}
break;
}
...
QString task3(QStringList params, int descriptor){
    ...
    if ( right =="+"){
        ...
        return "Ответ неверный, правильный ответ: " + QString::number(maxFlow);
    }
    return ans;
}
